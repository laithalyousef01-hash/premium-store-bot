...
def delivery_mode_keyboard(kind: str, record_id: int):
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton(TEXTS["en"]["structured_delivery"], callback_data=f"delmode|{kind}|{record_id}|structured"),
            InlineKeyboardButton(TEXTS["en"]["raw_delivery"], callback_data=f"delmode|{kind}|{record_id}|raw"),
        ]
    ])
...
def delivery_fields_keyboard(config_dict):
    ...
    return InlineKeyboardMarkup([
        ...
        [
            InlineKeyboardButton(TEXTS["en"]["continue"], callback_data="delfield_continue"),
        ],
    ])
...
async def reject_admin_access(target_message, user, context: ContextTypes.DEFAULT_TYPE, reason: str):
    ...
    await target_message.reply_text("⛔ Unauthorized admin action.")
...
async def handle_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "delfield_continue":
        ...
        await query.message.reply_text(f"{TEXTS['en']['send_field_value']} {next_field}:")
        return
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if (
        ...
        if not is_authorized_admin_context(message.chat_id, user_id):
            await reject_admin_access(message, user, context, "admin structured delivery submission")
            return
        value = message.text.strip()
        if not value:
            await message.reply_text(TEXTS["en"]["delivery_empty_value"])
            return
        ...
        if queue:
            ...
            context.user_data["admin_delivery_current_field"] = next_field
            await message.reply_text(f"{TEXTS['en']['send_field_value']} {next_field}:")
            return

        kind = context.user_data.get("admin_delivery_target_kind")
        target_id = context.user_data.get("admin_delivery_target_id")
        data = context.user_data.get("admin_delivery_data", {})
        formatted = format_structured_delivery_for_user(
            get_order(target_id)[1] if kind == "subscription" and get_order(target_id) else
            get_game_request(target_id)[1] if kind == "game" and get_game_request(target_id) else 0,
            data
        )
        ...
        if kind == "subscription":
            ...
            try:
                ...
                set_order_delivery(target_id, formatted)
                logger.info("Subscription delivered with structured data | order_id=%s | admin_user_id=%s", target_id, user_id)
                await message.reply_text(TEXTS["en"]["delivered_success_subscription"])
            except Exception as e:
                logger.exception("Failed structured delivery for subscription order | order_id=%s", target_id)
                await message.reply_text(TEXTS["en"]["failed_deliver_subscription"])
            ...
            try:
                ...
                set_game_request_delivery(target_id, formatted)
                logger.info("Game request delivered with structured data | request_id=%s | admin_user_id=%s", target_id, user_id)
                await message.reply_text(TEXTS["en"]["delivered_success_game"])
            except Exception as e:
                logger.exception("Failed structured delivery for game request | request_id=%s", target_id)
                await message.reply_text(TEXTS["en"]["failed_deliver_game"])
