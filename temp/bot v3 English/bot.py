...
def game_admin_paid_keyboard(request_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🎮 Deliver Game", callback_data=f"game_admin_deliver|{request_id}")]
    ])


def subscription_admin_paid_keyboard(order_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("📦 Deliver Subscription", callback_data=f"sub_admin_deliver|{order_id}")]
    ])
...
async def handle_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith("game_admin_price|"):
        ...
        await query.message.reply_text(
            f"Send the price now for request #G{request_id}.\n\nExample:\n20"
        )
        return
    ...
    if query.data.startswith("game_admin_deliver|"):
        ...
        await query.message.reply_text(
            f"Send the delivery details now for game request #G{request_id}.\n\n"
            f"Example:\n"
    ...
    if query.data.startswith("sub_admin_deliver|"):
        ...
        await query.message.reply_text(
            f"Send the delivery details now for subscription order #{order_id}.\n\n"
            f"Example:\n"
            f"Email: test@gmail.com\n"
            f"Password: 123456\n"
            f"Profile: 2\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # -------------------------
    # Admin: send price by plain number in orders group
    # -------------------------
    if (
    ...
    # -------------------------
    # Admin: deliver subscription from orders group
    # -------------------------
    if (
        message.chat_id == config.ORDERS_GROUP_ID
        ...
        try:
            await context.bot.send_message(
                chat_id=user_id,
                text=(
                    f"📦 Your subscription has been delivered successfully.\n\n"
                    f"Order ID: #{order_id}\n"
                    f"Product: {product}\n"
                    f"Plan: {plan}\n\n"
                    f"{delivery_text}\n\n"
            ...
            await message.reply_text(f"Subscription order #{order_id} delivered successfully.")
        except Exception as e:
            print("DELIVER SUBSCRIPTION ERROR:", e)
            await message.reply_text(f"Failed to deliver subscription order #{order_id}.")

        context.user_data.pop("admin_waiting_subscription_delivery_for_order", None)
        return

    # -------------------------
    # Admin: deliver game from orders group
    # -------------------------
    if (
        message.chat_id == config.ORDERS_GROUP_ID
        ...
        try:
            await context.bot.send_message(
                chat_id=user_id,
                text=(
                    f"🎮 Your game order has been delivered successfully.\n\n"
                    f"Request ID: #G{request_id}\n"
                    f"Game: {game_name}\n"
                    f"Platform: {platform}\n"
                    f"Plan: {plan}\n\n"
            ...
            await message.reply_text(f"Game request #G{request_id} delivered successfully.")
        except Exception as e:
            print("DELIVER GAME ERROR:", e)
            await message.reply_text(f"Failed to deliver game request #G{request_id}.")
